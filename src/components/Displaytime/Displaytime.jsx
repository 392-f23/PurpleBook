...
const Displaytime = (props) => {
    ...
    const queryBookHistory = (selectedDay, bookHistory) => {
      ...
      const days = bookHistory.filter(day => selectedDay.isSame(day.split(' ')[0] , 'day') ).map(slot => slot.split(' ')[1]);
      return days;
    };
        ...
        <ListGroup>
            {dayInfo.length === 0 ? (
                ...
                {/* {console.log(queryBookHistory(selectedDay, bookHistory))} */}
                  ...
                  {Object.entries(dayInfo[1]).filter(item => !item[1].booked && !queryBookHistory(selectedDay, bookHistory).includes(item[1].time)).map((item, index) => (
                      <ListGroup.Item key={index} action onClick={() => handleClick(item[1].time)}>
                        {item[1].time}
                      </ListGroup.Item>
                    ))
                  }
